...
- **Never bypass the figure generation pipeline.** Every figure must go through the full generation workflow: method selection from the tiered hierarchy, prompt preview (for AI-generated figures), structured critique (5 categories), and refinement loop. Do not hand-code raw inline SVGs, generate ad-hoc diagrams outside the pipeline, or skip the critique loop. The `/sbir:proposal-draft-figure` command exists for per-figure generation with quality control — use it or follow the same pipeline steps. Hand-coded inline SVG is permitted only when ALL of the following are true: (1) no generation tool (Mermaid, Graphviz, Python, Nano Banana, TikZ) is available for that figure type, (2) the figure is a simple diagram (not a concept image or illustration), and (3) the figure still goes through the structured critique loop after generation.
