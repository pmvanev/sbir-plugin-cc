...
4. **Style analysis (interactive -- MUST get user input)**: Read solicitation context (agency, domain, topic area). Match against the agency-domain style database in the visual-style-intelligence skill. Build a recommended style profile (palette, tone, detail level, avoid list). **Present the recommendation to the user using AskUserQuestion** with color hex codes, tone description, and rationale. Offer three options: **approve** (use as-is), **adjust** (user modifies palette, tone, detail level, or avoid items), **skip** (no style profile). **Do not proceed until the user selects an option.** Do not auto-approve or silently accept the recommendation. If the user adjusts, present the revised profile and confirm again. If no Nano Banana figures are planned, style analysis is optional -- inform the user and offer to proceed without a profile, but still ask explicitly. If the agency is not in the style database, recommend the generic professional style and inform the user. Record any user adjustments in the `user_adjustments` field. Persist the approved profile to `{artifact_base}/wave-5-visuals/style-profile.yaml` using the schema defined in the visual-style-intelligence skill.
5. **Method selection (interactive -- MUST discuss with user)**: For each planned figure, present the available generation methods based on tool detection results and the figure type. Use the tiered method hierarchy from the visual-asset-generator skill as a starting recommendation, but **present the options to the user and let them choose**. For each figure, show: the recommended method (with rationale), alternative methods that are available, and what each method is best at. Specifically ask whether to use **Nano Banana** (AI image generation) for concept figures, illustrations, and deployment scenarios -- do not silently skip it. If multiple viable methods exist, **use AskUserQuestion to let the user select**. Do not auto-assign methods without user confirmation.
6. For each planned figure, write a specification (type, user-selected method, purpose, cross-references, compliance items, caption) using the format from the visual-asset-generator skill
7. Write figure specifications to `{artifact_base}/wave-5-visuals/figure-specs.md`
8. **Figure plan review (interactive -- MUST get approval)**: Present the complete figure plan with all method selections, style profile summary, and cross-references. **Use AskUserQuestion** to offer: **approve** (proceed to Phase 2), **revise** (user provides feedback on specific figures or methods, loop back to adjust), **skip** (defer Wave 5). **Do not proceed to Phase 2 until the user explicitly approves the figure plan.**

Gate: Every figure has a specification. Methods confirmed by user. Cross-references verified against outline. Style profile persisted (or user explicitly chose to skip via AskUserQuestion).
...
-> Load visual-asset-generator skill. Run style analysis: present agency style recommendation, ask user to approve/adjust/skip. Then for each figure, present method options: "Architecture diagram: I recommend Mermaid (flowchart TB) since mmdc is available. Alternatives: inline SVG, external brief. Which method?" User selects Mermaid for both. Write specifications with user-selected methods. Present figure plan for approval. After approval, generate as `.mmd` files, render to SVG via `mmdc`. Apply style profile palette. Present each for structured critique.

### Example 2: Concept Figure with Nano Banana
Figure plan includes a deployment scenario illustration. `GEMINI_API_KEY` is set.

-> Load visual-asset-generator skill. Run style analysis with user. For the deployment figure, present options: "This is a concept illustration -- I recommend Nano Banana (AI image generation) since GEMINI_API_KEY is set. This will produce a professional illustration. Alternatives: external brief, inline SVG (not recommended for illustrations). Would you like to use Nano Banana?" User confirms. Write specification. After plan approval, craft engineered prompt, show preview to user, wait for confirm before generating.
...
-> First, flag the tool gap: "No generation tools detected. Recommend installing Mermaid CLI (`npm install -g @mermaid-js/mermaid-cli`) or setting `GEMINI_API_KEY` for higher-quality figures." For each figure, discuss options with user: "Block diagram -- only inline SVG is available. The result will be basic. Proceed with SVG, or defer to external brief?" "Concept image -- inline SVG cannot produce illustrations. I recommend an external brief. Proceed?" Write specs with user-confirmed methods. After plan approval, generate SVGs through structured critique loop. Track deferred figures as "pending-external" in figure log.
