...
Goal: Package the finalized proposal for portal submission with zero packaging errors, full verification, immutable archival, and confirmation capture -- all outputs to `{artifact_base}/wave-8-submission/`.
...
6. **Assembled package is the input, not the source sections**: Read from `{artifact_base}/wave-6-formatted/` (the formatter's assembled output). Do not re-read or re-assemble from draft sections -- the formatter already resolved formatting, compliance, and assembly.
...
1. Read proposal state from `{state_dir}/proposal-state.json` -- extract `topic.agency`, `topic.deadline`, `topic.solicitation_file`
...
### Phase 2: PACKAGE
1. Read the assembled proposal from `{artifact_base}/wave-6-formatted/`
...
7. Write packaged files to `{artifact_base}/wave-8-submission/staged/`
...
### Phase 3: VERIFY
1. Read compliance matrix from `{state_dir}/compliance-matrix.md`
...
5. Write verification report to `{artifact_base}/wave-8-submission/verification-report.md`
...
5. Write guide to `{artifact_base}/wave-8-submission/upload-guide.md`
...
2. Copy staged files to `{artifact_base}/wave-8-submission/submitted/` (immutable archive)
3. Compute SHA-256 checksums for every file in the archive
4. Write submission manifest to `{artifact_base}/wave-8-submission/submission-manifest.md` with:
   - Portal name, confirmation number, timestamp
   - Topic ID and title
   - File inventory with sizes and checksums
5. Write confirmation record to `{artifact_base}/wave-8-submission/confirmation/confirmation-receipt.md`
...
- Read the assembled package from `{artifact_base}/wave-6-formatted/`, not from draft sections. Re-assembling from drafts risks including unreviewed content.
...
User invokes submission agent but `{artifact_base}/wave-6-formatted/` does not exist or is empty.

-> Return error: "Assembled proposal package not found at `{artifact_base}/wave-6-formatted/`. Wave 8 submission requires the formatted and assembled package from Wave 6. Run the formatter agent for Wave 6 first." Do not attempt to package from draft sections.
