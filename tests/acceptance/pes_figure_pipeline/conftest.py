"""Acceptance test conftest -- fixtures for PES Figure Pipeline Enforcement BDD scenarios.
...
Evaluators receive pre-resolved tool_context (file_path + artifacts_present).
"""
...
# ---------------------------------------------------------------------------
# PES Config Fixture -- Real pes-config.json with figure pipeline rules
# ---------------------------------------------------------------------------


@pytest.fixture()
def pes_config_path(tmp_path: Path) -> Path:
    """Write the real pes-config.json with all 10 rules to a temp directory.

    Includes the 8 existing rules plus the 2 new figure pipeline rules:
    - figure-pipeline-requires-specs (figure_pipeline_gate)
    - figure-generation-requires-style (style_profile_gate)
    """
    config = {
        "rules": [
            {
...
def base_proposal_state() -> dict[str, Any]:
    """Minimal valid proposal state for SF25D-T1201 at Wave 5."""
    return {
