...
def audit_entry_includes_rule_id(
    ...
    The engine does not log rule_id explicitly; instead, block messages are
    generated by rule-specific evaluators and contain identifying text.
    Map rule_id to expected message fragments to verify rule participation.
    """
    # Map rule_ids to distinctive message fragments produced by their evaluators
