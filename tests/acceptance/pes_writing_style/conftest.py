"""Acceptance test conftest -- fixtures for PES Writing Style Gate Enforcement BDD scenarios.
...
- artifacts_present: local proposal artifacts (existing pattern)
- global_artifacts_present: global ~/.sbir/ artifacts (new for this feature)
"""
...
# ---------------------------------------------------------------------------
# PES Config Fixture -- Real pes-config.json with writing style gate rule
# ---------------------------------------------------------------------------


@pytest.fixture()
def pes_config_path(tmp_path: Path) -> Path:
    """Write the real pes-config.json with all 11 rules to a temp directory.

    Includes the 10 existing rules plus the new writing style gate rule:
    - drafting-requires-style-selection (writing_style_gate)
    """
    config = {
        "rules": [
            {
